function [d_BinCenter, NoiseCorr_histmean, NoiseCorr_histsem, par2, CI2, N_valid] =...
    NoiseCorr_Analysis_dtheta(FR, Pref_theta, dtheta_bound, d_BinStep_micron, d_tot, FR_thr)
...
dtheta_FR = triu_new(real(abs(bsxfun(@dtheta, Pref_theta(:), Pref_theta(:)'))), 0, 1);
if_dtheta_valid = (dtheta_FR >= dtheta_bound(1)) & (dtheta_FR <= dtheta_bound(2));
...
for stim_k = 1: N_stim
    ...
    %
    dtheta_FR = triu_new(real(abs(bsxfun(@dtheta, Pref_theta(idx_valid), Pref_theta(idx_valid)'))), 0, 1);
    if_dtheta_valid = (dtheta_FR >= dtheta_bound(1)) & (dtheta_FR <= dtheta_bound(2));
    %
    % Find distance
    ...
    distance = sqrt(bsxfun(@minus, idx_x', idx_x) .^ 2 +...
        bsxfun(@minus,  idx_y', idx_y) .^ 2);
    distance = triu_new(distance, 0, 1);
    distance = distance(if_dtheta_valid);
    %
    NoiseCorr = triu_new(corrcoef(FR_k(idx_valid, :)'), 0, 1);
    NoiseCorr = NoiseCorr(if_dtheta_valid);
