...
T_on = param.T_on; T_off = param.T_off; clear param
...
% sbatch --array=1-40% main.sh    % corresponding to Parset_i = 1;
...
FR = zeros(N, Nseg_valid);    % N x 50    % will be in Hz.
Isyn_tot = zeros(Nrecord, Nseg_valid, 3);    % N x 50 x 3    % inputs from F, E, I    % will be in V/s.
%
Tw_NumOff = floor(T_off / Tw);    % 1
Tw_NumOn = floor(T_on / Tw);    % 4
%
for seg_i = 1: Nseg_valid
    seg_k = Nseg - Nseg_valid + seg_i;
    Idx_on = (seg_k - 1) * (Tw_NumOff + Tw_NumOn) + [Tw_NumOff + 1: Tw_NumOff + Tw_NumOn];
    %
    FR(1: Ne, seg_k) = sum(E_Spkcount_tmp(:, Idx_on), 2) / (T_on / 1000);    % Hz
    FR(Ne + 1: N, seg_k) = sum(I_Spkcount_tmp(:, Idx_on), 2) / (T_on / 1000);
    Isyn_tot(:, seg_k, 1) = mean(Isyn(1: Nrecord, Idx_on), 2);    % V/s
    Isyn_tot(:, seg_k, 2) = mean(Isyn(Nrecord + 1: 2 * Nrecord, Idx_on), 2);
    Isyn_tot(:, seg_k, 3) = mean(Isyn(2 * Nrecord + 1: 3 * Nrecord, Idx_on), 2);
end
Isyn = Isyn_tot; clear Isyn_tot

% Spike counts of each simulation.
save([pwd, '/Results/Results_Par', num2str(Parset_i), '_Trial_Test.mat'], 'FR', 'Isyn', 'N_trial', 's_sample');
