...
load([dir_local, '/ParScan/sigma_narrow/sigma_narrow_mode', num2str(scan_mode), '.mat']);
%
switch scan_mode
case 1
y_min = -0.01; y_max = 0.04; x_max = 50;
y_txt = [0.14, 0.115, 0.09] * (y_max / 0.16);
dxy_full = 0.1; A_max = 0.02;
lambda_lb = 7.5 * sqrt(2); lambda_ub = 17.5 * sqrt(2); %lambda_lb = 11; lambda_ub = 25; 
case 2
y_min = -0.01; y_max = 0.04; x_max = 50;
y_txt = [0.14, 0.115, 0.09] * (y_max / 0.16);
dxy_full = 0.1; A_max = 0.02;
...
for j_plot = 1: Nx_plot
    ...
    text(15, y_txt(1), ['A = ', num2str(par_tmp(1), '%.3f')], 'FontSize', 5);
    text(15, y_txt(2), ['\lambda = ', num2str(par_tmp(2), '%.2f')], 'FontSize', 5);
    text(15, y_txt(3), ['b = ', num2str(par_tmp(3), '%.3f')], 'FontSize', 5);
...
pause(2); print(gcf, '-dpng', [dir_local,...
    '/ParScan/sigma_narrow/Plot_curves_sigma_narrow_mode', num2str(scan_mode), '.png']);
close;
...
%
figure; set(gcf, 'Units', 'Normalized', 'OuterPosition', [0, 0, 0.6, 1]); hold on;
suptitle(['\sigmaB = [', num2str(sigma_micron_B(1)), ', ', num2str(sigma_micron_B(2)),...
    ', ', num2str(sigma_micron_B(3)), '] (\mum); \kappa = [', num2str(kappa(1)), ', ',...
    num2str(kappa(2)), ', ', num2str(kappa(3)), ']'], 6, 0.96);
imagesc(x_list_full, y_list_full, lambda_clr); axis xy;
colormap(clb); clb = colorbar; axis square; caxis([lambda_lb lambda_ub]);
clb.Ticks = linspace(lambda_lb, lambda_ub, 5);
clb.TickLabels = linspace(lambda_lb / sqrt(2), lambda_ub / sqrt(2), 5);
%clb.Ticks = [lambda_lb: 2: lambda_ub];    % linspace(lambda_lb, lambda_ub, 7);
%axis([x_list(1), x_list(end), y_list(1), y_list(end)]);
%set(gca, 'XTick', 2.5: 2.5: x_list(end), 'YTick', 2.5: 2.5: y_list(end));
axis([2.5 12.5 2.5 12.5]);
set(gca, 'XTick', 2.5: 2.5: 12.5, 'YTick', 2.5: 2.5: 12.5);
xlabel(x_name); ylabel(y_name);
axftsize = 15; ax = gca; ax.FontSize = axftsize;
set(gca, 'Layer', 'top');
%
pause(2); print(gcf, '-dpng', [dir_local,...
    '/ParScan/sigma_narrow/Plot_colormap_sigma_narrow_mode', num2str(scan_mode), '.png']);
close;
