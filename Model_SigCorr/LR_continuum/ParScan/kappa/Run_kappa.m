...
if scan_mode == 1
    y_name = '\kappa\_E = \kappa\_I'; x_name = '\kappa\_F';
elseif scan_mode == 2
    y_name = '\kappa\_I'; x_name = '\kappa\_F = \kappa\_E';
end
kappa = [NaN, NaN, NaN];
y_list = 0: 0.01: 0.15; Ny = length(y_list);
x_list = 0: 0.01: 0.15; Nx = length(x_list);
%
...
for j = 1: Nx
    if scan_mode == 1
        kappa(2: 3) = y_list(i);    % kappa_E = kappa_I
        kappa(1) = x_list(j);    % kappa_F
    elseif scan_mode == 2
        kappa(3) = y_list(i);    % kappa_I
        kappa(1: 2) = x_list(j);    % kappa_F = kappa_E
    end
    ...
    try
        ...
        fprintf(['kappa_x = ', num2str(x_list(j)), ', kappa_y = ', num2str(y_list(i)), ' failed! \n']);
    end
end
fprintf([num2str(i), ' / ', num2str(Ny), ' end.\n']);
end
clear dir_0 kappa par_lb par_ub options A_amp i j Cov_L23E_ij Err IniVal par_tmp 
%
save([dir_local, '/ParScan/kappa/kappa_mode', num2str(scan_mode), '.mat'],...
    'Cov_L23E', 'Cov_L23E_par', 'd_micron', 'Exp2', 'fitpar_exp2_L4',...
    'Nx', 'Ny', 'sigma_micron', 'W', 'x_list', 'x_name', 'y_list', 'y_name');
