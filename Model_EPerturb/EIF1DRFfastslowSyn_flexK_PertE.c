/* To use function from matlab, first compile by entering this into the Matlab command window:
   mex EIF1DRF2tausyn_spc.c
   Then call the function like this:
   [s,Isyn]=EIF1DRFfastslowSyn_spc(sx, Wrf,Wrr,param,Tw); 
 */
...
/* sx is the feedforward presynaptic spike trains, which should be 3xNsx where Nsx is the number of spikes.
  ...
  Psyn(i,j): percentage of synapse j for (X, E, I (i=1,2,3))
   ...
   Tw is the time window to record the mean of Isyn. The size of full Isyn is (Nsyn * N,
        T/dt), while Isyn for each 0.05 time step is unnecessary. A modification is made to 
        record Isyn average in the time window of Tw.
   ...
   Isyn = [IsynX0; IsynE0; IsynI0] are the recorded 
   feedforward synaptic input, exc synaptic input, inh synaptic input respectively.
...
int Ntref[2],Ni,Ne,Nx,k,j,i,N,Nt,m1,m2,maxns,ns,Nrecord,jj,nskiprecord,tempflag,Nsx,Nw;
double dt,*s,*v,*v0,*Psyn;
double *Isyn, *Isynprime;
int *Wrr, *Wrf, *Pr; 
double *taudsyn,*taursyn,*sx,Jex,Jix,*iXspkInd;
double Jee,Jei,Jie,Jii,T,*Irecord,*C,*Vleak,*DeltaT,*VT,*tref,*gl,*Vth,*Vre,*Vlb,xloc,yloc;
int *refstate,iXspike,jspike,*postcellX,*postcellE,*postcellI,postcell, Nsyn, isyn, Nsyntype, *syntype; 
const mxArray  *mxTmp;
double  *temp1,*temp2,  Isyntot;

int Tw,Twm,Twn,NTw, NumTw;
double *IsynrecordTmp,*Isynrecord;
double *Kx, *Kr, KxMax, KeMax, KiMax, *KxCumsum;
int K1, K2, K3;

int itPert;
double *tPert, *PertIdx;
...
/* prhs[4]: time window length for Isyn average */
Tw = (int) mxGetScalar(prhs[4]);
Twm = mxGetM(prhs[4]);
Twn = mxGetN(prhs[4]);
if((Twm!=1)||(Twn!=1)){
    mexErrMsgTxt("Tw should be a 1x1 number.");
}
...
mxTmp = mxGetField(prhs[3],0,"Kx");
Kx = mxGetPr(mxTmp);
mxTmp = mxGetField(prhs[3],0,"KxCumsum");
KxCumsum = mxGetPr(mxTmp);
mxTmp = mxGetField(prhs[3],0,"KxMax");
KxMax = mxGetPr(mxTmp)[0];
// Kx: (1, Nx), post neuron idx (from 1 to N) of each pre neuon, including both e or i
// KxCumsum: tmp = cumsum(Kx); KxCumsum = [0 tmp(1: end - 1)];
// KxMax = max(Kx);
// eg. x x x x v v d d d
// Kx = [4, 2, 3]
// KxCumsum = [0 4 6], just the starting idx position (0-index)

mxTmp = mxGetField(prhs[3],0,"Kr");
Kr = mxGetPr(mxTmp);
mxTmp = mxGetField(prhs[3],0,"KeMax");
KeMax = mxGetPr(mxTmp)[0];
mxTmp = mxGetField(prhs[3],0,"KiMax");
KiMax = mxGetPr(mxTmp)[0];
...
mxTmp = mxGetField(prhs[3],0,"PertIdx");
PertIdx = mxGetPr(mxTmp);

mxTmp = mxGetField(prhs[3],0,"tPert");
tPert = mxGetPr(mxTmp);
...
plhs[1] = mxCreateDoubleMatrix(Nrecord*Nsyn, NumTw, mxREAL);
...
IsynrecordTmp = mxMalloc(Nsyn*Nrecord*sizeof(double));  /* mean synp. currents in the time window, NxNsyn   */ 
...
postcellX=mxMalloc((int)round(KxMax)*sizeof(int)); /* index for postsynaptic cells */
postcellE=mxMalloc((int)round(KeMax)*sizeof(int));
postcellI=mxMalloc((int)round(KiMax)*sizeof(int));
...
for(jj=0;jj<Nrecord;jj++){
  if(Irecord[jj]<1 || Irecord[jj]>N+1)
    ...
    for(isyn=0;isyn<Nsyn;isyn++){
        IsynrecordTmp[isyn*Nrecord+jj]=Isyn[(int)round(Irecord[jj]-1)*Nsyn+isyn];
    }
...
//for(i=1;i<100 && ns<maxns;i++){ 
    ...
    for(j=0;j<N;j++){      
      ...
      /* Spikes will be propagated at the END of the time bin (see below)*/
        ...
        if(j<Ne){
             ...
             if (i==(int)round(tPert[itPert]) && j==(int)round(PertIdx[itPert]-1)){
                 itPert++;
                 if(refstate[j]<=0){
                    Isyntot=0;
                    for(isyn=0;isyn<Nsyn;isyn++){
    ...
    for(jj=0;jj<Nrecord;jj++){
      if(Irecord[jj]<1 || Irecord[jj]>N+1)
         mexErrMsgTxt("Indices in Irecord must be between 1 and N");
        for(isyn=0;isyn<Nsyn;isyn++){
            /* IsynrecordTmp[isyn*Nrecord+jj+i*Nrecord*Nsyn]=Isyn[(int)round(Irecord[jj]-1)*Nsyn+isyn]; */
            IsynrecordTmp[isyn*Nrecord+jj]=IsynrecordTmp[isyn*Nrecord+jj]+Isyn[(int)round(Irecord[jj]-1)*Nsyn+isyn];
        }
      /* vr[jj+Nrecord*i]=v[(int)round(Irecord[jj]-1)]; */
    }

    /* Store Isynrecord at each end of time window  */
    if((i+1)%NTw==0){
        for(jj=0;jj<Nrecord;jj++){
            for(isyn=0;isyn<Nsyn;isyn++){
                Isynrecord[isyn*Nrecord+jj+(i/NTw)*Nrecord*Nsyn]=IsynrecordTmp[isyn*Nrecord+jj]/NTw;
                IsynrecordTmp[isyn*Nrecord+jj]=0;
            }
        }
    }

}  /* "end" of the whole time loop. */
